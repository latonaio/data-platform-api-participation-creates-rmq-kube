...
import (
	...
	"data-platform-api-participation-creates-rmq-kube/config"
	"encoding/json"
...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementParticipation(input, l)
	if err != nil {
		return xerrors.Errorf("complement participation error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementParticipation(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'Participation') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.Participation = &nr.LatestNumber
	return &nr, nil
}
